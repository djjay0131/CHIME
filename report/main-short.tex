...
\large CS 6204 Final Project Report \\[0.3em] \textit{(short version; full report at \texttt{main-partwo.pdf})}}
...
\paragraph{CloudLab control surface --- and what broke.}\label{sec:approach:cloudlab} CloudLab exposes two authentication surfaces that do not interoperate: an \emph{XML-RPC API} (\texttt{boss.emulab.net:43794}, PEM-cert auth) that an agent can drive, and a \emph{web portal} (\texttt{www.cloudlab.us}, JWT cookie) that only a human browser can. We wrapped the RPC side as Python (\texttt{script/cloudlab-setup.py}, \texttt{cloudlab-status-watch.sh}, \texttt{prep-experiment.sh}). What broke: the \texttt{reservationStatus} RPC returned a Perl error (\texttt{Undefined subroutine \&main::DoStatus} at \texttt{/usr/testbed/bin/manage\_resgroup} line 208) regardless of UUID or auth mode --- so the only programmatic path to inspect reservation state was gone, and the JWT-only fallback was structurally inaccessible to the agent. Detail in \S\ref{sec:postmortem}.
...
\end{lstlisting}
Regardless of UUID, auth mode, or CLI invocation. With \texttt{reservationStatus} broken, the only programmatic path to inspect reservation state was gone. The fallback (the web UI behind JWT auth) is structurally inaccessible to an agent without an interactive browser, because PEM-cert auth and JWT-cookie auth do not interoperate across the boss XML-RPC and \texttt{www.cloudlab.us} surfaces (\S\ref{sec:approach:cloudlab}).
