...
\note{
  ...
  \textbf{Say:} Sherman is built from CHIME's source tree with all five feature flags compiled out, so it shares CHIME's CXL transport. The 2x2 chart isolates "what does CXL transport buy you" from "what do CHIME's features add on top." Pattern is consistent: CXL beats RDMA on C and D for both methods, loses on E for both methods. But CHIME's CXL-over-RDMA speedup is larger than Sherman's --- about 3.4x at 4 threads versus Sherman's 1.4x. Reason: CHIME issues more small reads per op (hopscotch, sibling, speculative), each of which benefits independently from CXL's lower latency. Sherman's leaner profile has less latency to recover.\\
  \textbf{Transition:} Cleanest evidence yet that the CXL win is partly transport, partly features.\\
  \textbf{Time:} 60s
}
\note{
