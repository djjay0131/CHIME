...
\maketitle
\note{
  \textbf{Key point:} Introduction (open with humor; set up the two-contribution split).\\
  \textbf{Say:} Good morning, I'm Jason Cusati. Quick story before I start.
  Between April 22 and 26, I held four approved CloudLab reservations.
  None of them ever bound to an experiment. Six days of \texttt{0
  available because of existing resource reservations to other projects
  or users} --- the other project was, of course, mine. So I'm here today
  to thank CloudLab for accidentally inventing the most reliable
  denial-of-service against my own approved reservations. \emph{(beat)}
  That's the second contribution of this talk. The first is the one I
  actually planned: an agent-orchestrated reproduction pipeline on
  CloudLab, with CHIME, the SOSP 2024 paper, as the case study. Let me
  start with what CHIME is, then the orchestration layer, then the
  results.\\
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, text=mGray] at (-5, 0.4) {vs.\ \textbf{Rosendo et al., XLOOP '25}};
...
\note{
  ...
  \textbf{Transition:} Concrete wins from the orchestration.\\
  \textbf{Time:} 50s
}
\end{frame}

% Slide — Wins from agent orchestration (CUT-HERE-IF-SHORT, optional buffer)
% CUT-HERE-IF-SHORT  (drop cleanly if rehearsal > 13:00; jump straight to Key Takeaways)
\begin{frame}{What the Orchestration Actually Got Us}
\centering
\begin{tikzpicture}[
  win/.style={draw, rounded corners=4pt, minimum width=4.6cm,
              minimum height=1.6cm, align=center, font=\small,
              text width=4.4cm, fill=mGreen!12, draw=mGreen, inner sep=4pt},
]
  \node[win] (a) at (-3.6,  1.2) {\textbf{24-hr autonomous sweep}\\\scriptsize 9 phases, no human in chair};
  \node[win] (b) at ( 3.6,  1.2) {\textbf{Overnight monitoring}\\\scriptsize 29-min cadence, prompt-cache warm};
  \node[win] (c) at (-3.6, -1.0) {\textbf{Crash trace into NFS}\\\scriptsize captured the one-line allocator bug};
  \node[win] (d) at ( 3.6, -1.0) {\textbf{Spec chain across pivots}\\\scriptsize survived 3 reality changes};
\end{tikzpicture}
\note{
  \textbf{Key point:} Concrete value the agent layer delivered.\\
  \textbf{Say:} Four wins worth naming. First, a 24-hour autonomous
  sweep on May 2 --- nine phases, no human in the chair. Second,
  overnight monitoring on a twenty-nine-minute cadence, kept under
  the prompt-cache window so each wakeup was cheap. Third, the
  cluster-side runner wrote the crash trace into NFS, which is how
  the one-line allocator bug got found offline. And fourth, the spec
  chain --- four feature specs across three reality pivots --- held
  the project together when the plan changed twice. None of those
  required me to be awake at the right moment.\\
  \textbf{Transition:} The flip side --- where the approach broke.\\
  \textbf{Time:} 45s
}
\end{frame}

% Slide — Limitations of agent orchestration (CUT-HERE-IF-SHORT, optional buffer)
% CUT-HERE-IF-SHORT  (drop cleanly if rehearsal > 13:00; jump straight to Key Takeaways)
\begin{frame}{What the Orchestration Could Not Do}
\centering
\begin{tikzpicture}[
  lim/.style={draw, rounded corners=4pt, minimum width=4.6cm,
              minimum height=1.6cm, align=center, font=\small,
              text width=4.4cm, fill=mRed!10, draw=mRed, inner sep=4pt},
]
  \node[lim] (a) at (-3.6,  1.2) {\textbf{Bridge PEM/JWT split}\\\scriptsize web UI is browser-only};
  \node[lim] (b) at ( 3.6,  1.2) {\textbf{Replace human judgment}\\\scriptsize reservations, force-pushes, triage};
  \node[lim] (c) at (-3.6, -1.0) {\textbf{Diagnose server-side bugs}\\\scriptsize broken \texttt{reservationStatus} blocks all};
  \node[lim] (d) at ( 3.6, -1.0) {\textbf{Prove productivity claims}\\\scriptsize no human-only baseline};
\end{tikzpicture}
\note{
  \textbf{Key point:} Honest limits of the approach.\\
  \textbf{Say:} Four honest limits. First, the PEM-versus-JWT
  authentication split: the web UI is browser-only, no agent can
  bridge it without a real browser session. Second, irreversible
  decisions still went through me --- reservation requests, force
  pushes, control-net quarantine triage. Third, server-side bugs
  like the broken \texttt{reservationStatus} block automation
  completely, with no client-side workaround. And fourth, I have
  no controlled human-only baseline, so I cannot defend any
  quantitative productivity claim.\\
  \textbf{Transition:} Three closing points.\\
  \textbf{Time:} 45s
}
\end{frame}

% Slide — Key Takeaways (Zen rewrite: short punchy lines, prose in note)
\begin{frame}{Key Takeaways}
\centering
\begin{tikzpicture}[
  item/.style={font=\Large, anchor=west, text width=11cm},
  num/.style={font=\Huge\bfseries, text=myRed},
]
  \node[num]  at (-6.0, 2.0) {1};
  \node[item] at (-5.0, 2.0) {Agent substrate works at solo-PhD scale};
  \node[num]  at (-6.0, 0.4) {2};
  \node[item] at (-5.0, 0.4) {Three CloudLab API fixes --- each enough};
  \node[num]  at (-6.0,-1.2) {3};
  \node[item] at (-5.0,-1.2) {CXL: reproducible, workload-dependent};
\end{tikzpicture}
\note{
  \textbf{Key point:} Three closing points, leading with orchestration.\\
  \textbf{Say:} Three takeaways. First, the agent substrate ---
  persona-driven, memory-bank-anchored, not self-orchestrating ---
  works at solo-PhD scale. Second, the control-surface gaps on a
  shared academic testbed are themselves the contribution: three
  CloudLab API fixes, each independently sufficient. Third, on
  CHIME itself: reproduction trends match the paper, Sherman's LOAD
  crash proves the techniques are jointly necessary for correctness,
  and the CXL win is workload-dependent and more reproducible across
  days than RDMA.\\
  \textbf{Transition:} Wins, limits, and questions follow.\\
  \textbf{Time:} 45s
}
\end{frame}

% Slide — Questions (last spoken slide; thanks + open floor)
{
\setbeamercolor{background canvas}{bg=myRed}
\begin{frame}[plain]
\vspace*{\fill}
\begin{center}
{\color{white}\Huge\bfseries Questions?}\\[1.2em]
{\color{white}\large \textit{Thanks for listening.}}
\end{center}
\vspace*{\fill}
\note{
  \textbf{Key point:} Open floor; references reachable in backup deck.\\
  \textbf{Say:} Thank you. Happy to take questions on any layer of the
  pipeline, on the CHIME findings, or on the CloudLab postmortem ---
  the backup deck has the timeline, the four-method comparison, and
  the full citations if anything needs follow-up.\\
  \textbf{Transition:} ---\\
